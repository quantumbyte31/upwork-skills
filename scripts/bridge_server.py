"""Upwork Extension Bridge Server
...
logger = logging.getLogger("upwork-bridge")
    ...
    async def _handle_cli(self, ws: ServerConnection, msg: dict) -> None:
        ...
        if not self._extension_ws:
            err = (
                ...
                "the Upwork Bridge extension installed and enabled."
            )
...
async def main(port: int) -> None:
    ...
    async with websockets.serve(server.handle, "localhost", port):
        ...
        logger.info("Waiting for Upwork Bridge extension to connect...")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Upwork Extension Bridge Server")
    parser.add_argument("--port", type=int, default=9337, help="Listen port (default: 9337)")
