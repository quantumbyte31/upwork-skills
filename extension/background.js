/**
 * Upwork Bridge - Background Service Worker
 *
 * NETWORK AUDIT (exhaustive list of all network endpoints in this file):
 *   1. WebSocket to ws://localhost:9337 ONLY (BRIDGE_URL)
 *   2. chrome.cookies.getAll for domain "upwork.com" (handleCommand)
 *   3. chrome.tabs operations scoped to upwork.com host_permissions
...
const BRIDGE_URL = "ws://localhost:9337";
let ws = null;
...
function connect() {
  ...
  ws.onopen = () => {
    console.log("[Upwork Bridge] Connected");
  ...
  switch (method) {
    ...
    case "get_cookies":
      return await chrome.cookies.getAll({
        domain: params.domain || "upwork.com",
      });
    case "evaluate":
...
async function cmdNavigate({ url }) {
  const tab = await getOrOpenUpworkTab();
...
async function getOrOpenUpworkTab() {
  const tabs = await chrome.tabs.query({
    url: ["https://www.upwork.com/*"],
  });
  if (tabs.length > 0) return tabs[0];
  const tab = await chrome.tabs.create({ url: "https://www.upwork.com/nx/find-work/" });
